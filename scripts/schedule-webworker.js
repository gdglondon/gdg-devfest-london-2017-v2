  ...
  for (const dayKey of Object.keys(scheduleRaw)) {
    ...
    for (let timeslotsIndex = 0, timeslotLen = day.timeslots.length; timeslotsIndex < timeslotLen; timeslotsIndex++) {
      ...
      var timeslotGridSize = `${startGrid} / 1 / ${endGrid} / 2`;
        ...
        for (let sessionIndex = 0, sessionsLen = timeslot.sessions ? (timeslot.sessions.length ? timeslot.sessions.length : 5) : 0; sessionIndex < sessionsLen; sessionIndex++) {
          let subsessions = [];
          ...
          if (timeslot.sessions.length || timeslot.sessions[sessionIndex]) {
            ...
            for (let subSessionIndex = 0, subSessionsLen = timeslot.sessions[sessionIndex] && timeslot.sessions[sessionIndex].items ? timeslot.sessions[sessionIndex].items.length : 0; subSessionIndex < subSessionsLen; subSessionIndex++) {
            ...
            let columnEnd = timeslot.welcome ? 5 : sessionsLen !== 1 ? sessionIndex + 2 : Object.keys(extensions).length ? Object.keys(extensions)[0] : tracksNumber + 1;
